...
sy::ReturnNode::init(ParseInfo* parseInfo, DynArray<StackVariable>* variables, Scope*) noexcept {
    sy_assert(parseInfo->tokenIter.current().tag() == TokenType::ReturnKeyword,
              "Expected return keyword");
    sy_assert(this->retValue.hasValue() == false,
              "Should not already have an expression value to return");
...
Result<void, CompileError>
sy::ReturnNode::compileStatement(FunctionBuilder* builder) const noexcept {
    if (this->retValue.hasValue() == false) {
        ...
        const operators::ReturnValue ret = {
            static_cast<uint64_t>(operators::ReturnValue::OPCODE),
...
//     const auto load = *reinterpret_cast<const
//     operators::LoadImmediateScalar*>(&builder.bytecode[0]); CHECK_EQ(load.scalarTag,
//     static_cast<uint64_t>(ScalarTag::Bool)); CHECK_EQ(load.immediate,
//     static_cast<uint64_t>(true)); CHECK_EQ(builder.bytecode[1].getOpcode(), OpCode::ReturnValue);
// }
...
//     operators::LoadImmediateScalar*>(&builder.bytecode[0]); CHECK_EQ(load.scalarTag,
//     static_cast<uint64_t>(ScalarTag::Bool)); CHECK_EQ(load.immediate,
//     static_cast<uint64_t>(false)); CHECK_EQ(builder.bytecode[1].getOpcode(),
//     OpCode::ReturnValue);
// }
