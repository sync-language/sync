...
{
    return reinterpret_cast<Header**>(&this->hashMasks_[this->capacity_]);
}
...
void Group::Header::destroyKeyValue(
    sy::Allocator alloc,
    ...
    size_t valueSize)
{
    ...
    const size_t allocAlign = [keyAlign, valueAlign](){
        if(keyAlign > valueAlign) {
            if(keyAlign > alignof(Header)) return keyAlign;
            return alignof(Header);
        }
        else {
            if(valueAlign > alignof(Header)) return valueAlign;
            return alignof(Header);
        }
    }();
    alloc.freeAlignedArray(asBytes, allocSize, allocAlign);
...
void Group::Header::destroyScriptKeyValue(sy::Allocator alloc, const sy::Type *keyType, const sy::Type *valueType)
{
    ...
    const size_t byteOffsetForValue = byteOffsetForAlignedMember(
        byteOffsetForKey + keyType->sizeType, valueType->alignType);
    uint8_t* asBytes = reinterpret_cast<uint8_t*>(this);
    ...
    const size_t allocAlign = [keyType, valueType]() -> size_t {
        if(keyType->alignType > valueType->alignType) {
            if(keyType->alignType > alignof(Header)) return keyType->alignType;
            return alignof(Header);
        }
        else {
            if(valueType->alignType > alignof(Header)) return valueType->alignType;
            return alignof(Header);
        }
    }();
    alloc.freeAlignedArray(asBytes, allocSize, allocAlign);
...
sy::AllocExpect<Group::Header*> Group::Header::createKeyValue(
    sy::Allocator alloc,
    size_t keyAlign,
    size_t keySize,
    size_t valueAlign,
