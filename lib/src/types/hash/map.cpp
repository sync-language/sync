...
// sy::AllocExpect<bool> sy::RawMapUnmanaged::insert(Allocator& alloc, void* optionalOldValue, void* key, void* value, size_t (*hash)(const void* key),
...
sy::AllocExpect<void> sy::RawMapUnmanaged::ensureCapacityForInsert(Allocator& alloc) {
    ...
    if(this->available_ != 0) return sy::AllocExpect<void>(std::true_type{});;

    const size_t newGroupCount = [](size_t currentGroupCount) -> size_t {
        constexpr size_t DEFAULT_GROUP_COUNT = 1;
        if(currentGroupCount == 0) return DEFAULT_GROUP_COUNT;
    ...
    for(size_t oldGroupIter = 0; oldGroupIter < this->groupCount_; oldGroupIter++) {
        ...
        if(oldGroup.itemCount() == 0) continue;

        for(uint32_t hashMaskIter = 0; hashMaskIter < oldGroup.capacity(); hashMaskIter++) {
            if(oldGroup.hashMasksAsBytes()[hashMaskIter] == 0) continue;
