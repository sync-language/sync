...
std::optional<uint32_t> sy::Frame::frameExtendAmountForAlignment(const uint32_t totalSlots,
...
std::optional<std::tuple<Frame, const Bytecode*>> sy::Frame::readFromMemory(const uint64_t* valuesMem,
    ...
    oldFrame.frameLength = static_cast<uint16_t>(frameLengthAndFunctionIndex & 0xFFFF);
    oldFrame.functionIndex = static_cast<uint16_t>(frameLengthAndFunctionIndex >> 16);
    oldFrame.retValueDst = oldRetDst;
    return std::make_tuple(oldFrame, oldInstructionPointer);
}

void sy::Frame::storeInMemory(uint64_t* valuesMem, uintptr_t* typesMem, const Bytecode* instructionPointer) const {
    ...
    *frameLengthAndFunctionIndex =
        static_cast<uint64_t>(this->frameLength) | (static_cast<uint64_t>(this->functionIndex) << 16);
...
void sy::Frame::storeNullFrameInMemory(uint64_t* valueMem, uintptr_t* typesMem) {
...
const Bytecode* sy::Frame::readOldInstructionPointer(const size_t* valuesMem) {
    return reinterpret_cast<const Bytecode*>(valuesMem[OLD_INSTRUCTION_POINTER]);
}

void sy::Frame::storeOldInstructionPointer(size_t* valuesMem, const Bytecode* instructionPointer) {
