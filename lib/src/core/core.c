    ...
    (defined(__GNUC__) && !defined(__APPLE__) && !defined(_WIN32) && !defined(__EMSCRIPTEN__))
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
#endif
...
SY_API void sy_set_fatal_error_handler(void (*errHandler)(const char* message)) {
    sy_assert_release(errHandler != NULL,
...
SY_API void sy_set_write_string_error(void (*writeStrErr)(const char* message)) {
    sy_assert_release(writeStrErr != NULL,
...
void sy_make_pages_read_write(void* pagesStart, size_t len) {
    ...
    DWORD oldProtect;
    const bool success = VirtualProtect(pagesStart, len, newProtect, &oldProtect);
    sy_assert_release(success == true,
...
#else
    return atomic_fetch_add_explicit((_Atomic volatile size_t*)&self->value, toAdd,
                                     sy_memory_order_to_std(order));
#endif
}
...
#else
    return atomic_fetch_sub_explicit((_Atomic volatile size_t*)&self->value, toSub,
                                     sy_memory_order_to_std(order));
#endif
}
...
#else
    return atomic_exchange_explicit((_Atomic volatile size_t*)&self->value, newValue,
                                    sy_memory_order_to_std(order));
#endif
}
...
#if defined(_MSC_VER) && defined(__STDC_NO_ATOMICS__)
    (void)order;
    LONG64 prev = _InterlockedCompareExchange64((volatile LONG64*)&self->value, (LONG64)desired,
                                                (LONG64)*expected);
...
#else
    return atomic_compare_exchange_weak_explicit((_Atomic volatile size_t*)&self->value, expected,
                                                 desired, sy_memory_order_to_std(order),
...
#else
    return atomic_exchange_explicit((_Atomic volatile bool*)&self->value, newValue,
                                    sy_memory_order_to_std(order));
#endif
}

bool sy_atomic_bool_compare_exchange_weak(SyAtomicBool* self, bool* expected, bool desired,
                                          SyMemoryOrder order) {
#if defined(_MSC_VER) && defined(__STDC_NO_ATOMICS__)
    (void)order;
    ...
    char prev =
        _InterlockedCompareExchange8((volatile char*)&self->value, desiredChar, expectedChar);
...
#else
    return atomic_compare_exchange_weak_explicit((_Atomic volatile bool*)&self->value, expected,
                                                 desired, sy_memory_order_to_std(order),
                                                 memory_order_relaxed);
#endif
}
...
bool sy_relative_to_absolute_path(const char* relativePath, size_t relativePathLen,
                                  char* outAbsolutePath, size_t outAbsoluteBufSize) {
#if defined(_WIN32)
...
void print_windows_callstack(void) {
    ...
    for (WORD i = 2; i < frames; i++) {
        ...
        if (fileName != NULL && lineNumber != 0) {
            if (modName != NULL) {
                if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d %s at %s:%lu (in %s)", (int)i,
                             funcName, fileName, (unsigned long)lineNumber,
                             modName) > (int)sizeof(lineBuf)) {
                    success = false;
                }
            } else {
                if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d %s at %s:%lu", (int)i, funcName,
                             fileName, (unsigned long)lineNumber) > (int)sizeof(lineBuf)) {
                    success = false;
                }
            }
        } else if (modName != NULL) {
            if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d 0x%llx %s (in %s)", (int)i,
                         (unsigned long long)address, funcName, modName) > (int)sizeof(lineBuf)) {
                success = false;
            }
        } else {
            if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d 0x%llx %s", (int)i,
                         (unsigned long long)address, funcName) > (int)sizeof(lineBuf)) {
                success = false;
            }
        }
...
static void print_posix_callstack(void) {
    ...
    for (int i = 2; i < traceSize; i++) {
        ...
        if (dladdr(addresses[i], &info) == 0 || info.dli_fname == NULL) {
            if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d %p", i - 1, addresses[i]) <
                (int)sizeof(lineBuf)) {
                syncWriteStringError(lineBuf);
            } else {
                syncWriteStringError("???");
        ...
        if (snprintf(loadAddrStr, sizeof(loadAddrStr), "0x%llx",
                     (unsigned long long)(uintptr_t)(info.dli_fbase)) >= (int)sizeof(loadAddrStr)) {
            fallback_dladdr_print(&info, i, addresses);
            continue;
        }
        if (snprintf(addrStr, sizeof(addrStr), "0x%llx",
                     (unsigned long long)(uintptr_t)(addresses[i])) >= (int)sizeof(addrStr)) {
            fallback_dladdr_print(&info, i, addresses);
            continue;
        }

        const char* argv[] = {"/usr/bin/atos", "-o",        info.dli_fname, "-l",
        ...
        pid_t pid;
        int spawnResult =
            posix_spawn(&pid, "/usr/bin/atos", &actions, NULL, (char* const*)argv, environ);
        (void)posix_spawn_file_actions_destroy(&actions);
        if (spawnResult != 0) {
            fallback_dladdr_print(&info, i, addresses);
            (void)close(pipefd[0]);
        ...
        while (totalRead < (ssize_t)(sizeof(atosBuf) - 1) &&
               (bytesRead = read(pipefd[0], atosBuf + totalRead,
                                 sizeof(atosBuf) - 1 - (size_t)totalRead)) > 0) {
            totalRead += bytesRead;
        }
        atosBuf[totalRead] = '\0';
        (void)close(pipefd[0]);
        ...
        char offsetStr[64] = {0};
        if (snprintf(offsetStr, sizeof(offsetStr), "0x%llx", (unsigned long long)offset) >=
            (int)sizeof(offsetStr)) {
            fallback_dladdr_print(&info, i, addresses);
            continue;
        }

        const char* argv[] = {
            "/usr/bin/addr2line", "-e", info.dli_fname, "-f", "-C", "-p", offsetStr, NULL};
        ...
        pid_t pid;
        int spawnResult =
            posix_spawn(&pid, "/usr/bin/addr2line", &actions, NULL, (char* const*)argv, environ);
        (void)posix_spawn_file_actions_destroy(&actions);
        if (spawnResult != 0) {
            fallback_dladdr_print(&info, i, addresses);
            (void)close(pipefd[0]);
        ...
        while (totalRead < (ssize_t)(sizeof(addr2lineBuf) - 1) &&
               (bytesRead = read(pipefd[0], addr2lineBuf + totalRead,
        ...
        if (!isUnknown) {
            if (snprintf(lineBuf, sizeof(lineBuf), "#%-2d %s", i - 1, addr2lineBuf) <
                (int)sizeof(addr2lineBuf)) {
                syncWriteStringError(lineBuf);
            } else {
                fallback_dladdr_print(&info, i, addresses);
            }
...
#endif
        bool expected = false;
        while (!(sy_atomic_bool_compare_exchange_weak(&callstackMutex, &expected, true,
                                                      SY_MEMORY_ORDER_SEQ_CST))) {
            expected = false;
            sy_thread_yield();
        }
