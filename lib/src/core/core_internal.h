...
/// `SYNC_CUSTOM_ALIGNED_MALLOC_FREE`, in which you must supply a definition of the function at
/// final link time.
...
/// @return On success, returns the pointer to the beginning of the newly allocated memory. Must be
/// freed with `sy_aligned_free`. On failure returns a null pointer.
/// @warning If `len` is not a multiple of `align`, or `align` is not a power of `2`, the fatal
...
/// Allocates one or more pages of memory. If `SYNC_NO_PAGES` is defined, calls `sy_aligned_malloc`
/// with an `align` of `SYNC_DEFAULT_PAGE_ALIGNMENT` found in `core.c`. Alternatively, can be
...
/// @return On success, returns the pointer to the beginning of the newly allocated pages / memory.
/// Must be freed with `sy_page_free`. On failure returns a null pointer.
/// @warning If `len` is not a multiple of `sy_page_size` the fatal error handler is
/// invoked.
extern void* sy_page_malloc(size_t len);

/// Frees one or more pages of memory. If `SYNC_NO_PAGES` is defined, calls `sy_aligned_free` with
/// an `align` of `SYNC_DEFAULT_PAGE_ALIGNMENT` found in `core.c`. Alternatively, can be overridden
/// by defining `SYNC_CUSTOM_PAGE_MEMORY`
...
/// If `SYNC_NO_PAGES` is defined, uses `SYNC_DEFAULT_PAGE_ALIGNMENT` found in `core.c`.
...
/// Makes one or more virtual memory pages read only. If `SYNC_NO_PAGES` is defined, does nothing.
/// Alternatively, can be overridden by defining `SYNC_CUSTOM_PAGE_MEMORY`
...
/// Makes one or more virtual memory pages read / write enabled. If `SYNC_NO_PAGES` is defined, does
/// nothing. Alternatively, can be overridden by defining `SYNC_CUSTOM_PAGE_MEMORY`
/// @param pagesStart Pointer to the beginning of the pages memory.
/// @param len Amount of bytes that the pages span.
/// @warning If `len` is not a multiple of `sy_page_size` the fatal error handler is invoked.
...
/// Yields execution of this thread. Can be overridden by defining `SYNC_CUSTOM_THREAD_YIELD` and
/// then linking your own.
extern void sy_thread_yield(void);

// /// Enjoy reading
// /// As sync code executes, it will inevitably call into external functions (C functions) due to
// being embeddable.
// /// As such, some static analysis between external and sync function calls for the compiler is
// not fully possible.
// ///
// /// This presents a massive challenge when trying to acquire locks, as we cannot prevent, at sync
// compile time, that
// /// acquiring an exclusive lock has not already been acquired by that same thread as a shared
// lock.
// ///
// /// How we can solve this is by combining two methods. Firstly, supporting re-entrant locks. This
// will mean that a
// /// thread trying to re-acquire a lock in the same way as it did before is not a problem.
// Secondly, allow to "elevate" a
// /// lock, meaning turn a shared lock into an exclusive lock without giving up acquisition to
// another thread. If a thread
...
// /// Attempts to acquire a shared lock to `self`. If the calling thread already has an exclusive
// or shared lock, the lock
// /// re-enters and succeeds. Release the lock with `sy_raw_rwlock_release_shared`.
// /// @param self Non-null pointer to lock object
// /// @return If the acquire was a success, `SY_ACQUIRE_ERR_NONE`. If another thread has an
// exclusive lock,
// /// `SY_ACQUIRE_ERR_SHARED_HAS_EXCLUSIVE`. If memory failed to allocate,
// `SY_ACQUIRE_ERR_OUT_OF_MEMORY`.
// /// @warning `sy_raw_rwlock_release_shared` must be called to release the lock. Failure to do so
// is undefined behavior. SyAcquireErr sy_raw_rwlock_try_acquire_shared(SyRawRwLock* self);

// /// Acquires a shared lock to `self`. If the calling thread already has an exclusive lock, the
// lock re-enters
...
// /// @warning `sy_raw_rwlock_release_shared` must be called to release the lock. Failure to do so
// is undefined behavior. SyAcquireErr sy_raw_rwlock_acquire_shared(SyRawRwLock* self);

// /// Releases a shared lock to `self` from this thread. If there are multiple shared acquisitions
// on this thread,
// /// `sy_raw_rwlock_release_shared` must be called the same amount of times.
// /// @param self Non-null pointer to lock object
// /// @warning It is a fatal error if a different thread has an exclusive lock on `self`, or if no
// thread has a shared
// /// lock.
// void sy_raw_rwlock_release_shared(SyRawRwLock* self);

// /// Attempts to acquire an exclusive lock to `self`. If the calling thread already has an
// exclusive lock, the
// /// lock re-enters and succeeds. If no other thread has a shared lock, and this thread does, also
// the lock re-enters
...
// /// `SY_ACQUIRE_ERR_EXCLUSIVE_HAS_OTHER_READERS`. If two or more threads are trying to elevate
// from shared to exclusive,
// /// `SY_ACQUIRE_ERR_DEADLOCK`. If memory allocation fails, `SY_ACQUIRE_ERR_OUT_OF_MEMORY`.
// /// @warning `sy_raw_rwlock_release_exclusive` must be called to release the lock. Failure to do
// so is undefined
// /// behavior. It is best to not re-try the lock if `SY_ACQUIRE_ERR_EXCLUSIVE_HAS_OTHER_READERS`
// is returned,
// /// as this can easily lead to deadlocks. If `SY_ACQUIRE_ERR_DEADLOCK` is returned, two threads
// are competing to elevate
...
// /// `sy_raw_rwlock_release_exclusive`.
// /// @param self Non-null pointer to lock object
// /// @return If the acquire was a success, `SY_ACQUIRE_ERR_NONE`. If another thread is also trying
// to elevate their lock
// /// from shared to exclusive, `SY_ACQUIRE_ERR_DEADLOCK`. If memory allocation fails,
// `SY_ACQUIRE_ERR_OUT_OF_MEMORY`.
// /// @warning `sy_raw_rwlock_release_exclusive` must be called to release the lock. Failure to do
// so is undefined
// /// behavior. This function also does not re-try the lock if
// `SY_ACQUIRE_ERR_EXCLUSIVE_HAS_OTHER_READERS` is returned,
// /// as this can easily lead to deadlocks. For instance, if thread A and B both have shared locks,
// and then both want to
// /// elevate them to exclusive, this would be a deadlock. If `SY_ACQUIRE_ERR_DEADLOCK` is
// returned, two threads are
// /// competing to elevate their locks to exclusive. Analyze carefully where this is happening at
// the call-site. SyAcquireErr sy_raw_rwlock_acquire_exclusive(SyRawRwLock* self);
...
// /// @param self Non-null pointer to lock object
// /// @warning It is a fatal error if a different thread has an exclusive lock on `self`, or if no
// thread has an exclusive
// /// lock.
// void sy_raw_rwlock_release_exclusive(SyRawRwLock* self);

#ifndef SYNC_NO_FILESYSTEM

/// Fetches relevant file info from a file on disk. This functionality is not strictly required for
/// Sync to work, but is simply useful. The compiler can function without it if you do not call any
/// file system specific compiler functionality. Can be overridden by defining
/// `SYNC_CUSTOM_GET_FILE_INFO`, in which you must supply a definition of the function at final link
/// time. Since it's not required the compiler, you can link a function that just returns false, and
/// not use any file system logic for the compiler.
...
/// Converts a relative path to an absolute path. This functionality is not strictly required for
/// Sync to work, but it is simply useful. The compiler can function without it if you do not call
/// any file system specific compiler functionality. Can be overridden by defining
/// `SYNC_CUSTOM_RELATIVE_TO_ABSOLUTE_PATH`, in which you must supply a definition of the function
/// at final link time. Since it's not required the compiler, you can link a function that just
/// returns false, and not use any file system logic for the compiler.c
/// @param relativePath Non-null pointer to the relative path string. Does not need to be null
/// terminated.
/// @param relativePathLen Length, in bytes, of the string pointed to by `relativePath`.
/// @param outAbsolutePath Non-null pointer where the absolute path will be written to. Will have a
/// null terminator written to the end.
...
extern bool sy_relative_to_absolute_path(const char* relativePath, size_t relativePathLen,
...
#define always(expression)                                                                         \
    (!!(expression) ? 1                                                                            \
                    : (syncFatalErrorHandlerFn("always(" #expression ") failed" __FILE__           \
                                               ":" SY_STRINGIFY(__LINE__)),                        \
                       0))

/// The expression is assumed to never happen.
/// @param expression The boolean expression
#define never(expression)                                                                          \
    (!!(expression) ? (syncFatalErrorHandlerFn("never(" #expression ") triggered" __FILE__         \
                                               ":" SY_STRINGIFY(__LINE__)),                        \
                       1)                                                                          \
                    : 0)
#else
...
/// If `SYNC_LIB_CODE_COVERAGE` is defined, will forcefully create a branch. Then, when code
/// coverage is done, should that branch not have been executed.
